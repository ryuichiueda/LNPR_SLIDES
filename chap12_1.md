...
<img width="45%" src="./figs/non_qmdp.gif" />
<img width="45%" src="./figs/qmdp.gif" />
...
<img width="30%" src="./figs/one_landmark_qmdp.gif" />
<img width="65%" src="./figs/12.4.jpg" />

---

## 12.3.2 価値で重みをつけた<br />Q-MDP

* ゴールに近い（価値が高い）パーティクルに大きな決定権を与える
    * ロボットの挙動
        * ゴールに近いパーティクルの姿勢が正しいと仮定して行動
        * ゴールできなくても仮定の根拠のパーティクルがゴールに入って消える$\rightarrow$次の仮定を試す<br />　
* 価値の期待値の式
    * $Q_\text{PFC}(a,b) = \sum_{i=0}^{N-1} \dfrac{w^{(i)}}{[V_{\max}- V(s^{(i)})]^m} Q(s^{(i)},a)$
        * $i$: パーティクルのインデックス
        * $m>0$で、ゴールに近いパーティクルの「意見」が強くなる
            * 書籍では$m=2$

---

### 価値で重みをつけたQ-MDPでのロボットの挙動

* 動きに一貫性が出る<br />
（動画は次ページ）<br /><br /><br /><br /><br /><br /><br /><br />　

<img width="45%" src="./figs/12.5.jpg" />

---

### 価値で重みをつけたQ-MDPでのロボットの挙動（動画）
...
* Q-MDP
    ...
    * 少し式をいじるとロボットにゴールを探索させることができる<br />　
* 問題
    * 最適方策ではない
        * 途中で止まる
    * <span style="color:red">行動が観測に有利なのかどうかを考慮していない</span>
        * 後半に続く
